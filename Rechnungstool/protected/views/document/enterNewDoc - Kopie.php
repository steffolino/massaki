...
CustomEditor.prototype.close = function() {
  ...
  this.instance.removeHook('beforeKeyDown', onBeforeKeyDown);
	...
	var onBeforeKeyDown = function(event) {
		...
		// console.log(Handsont
		Handsontable.Dom.enableImmediatePropagation(event);

		switch (event.keyCode) {
			case (Handsontable.helper.keyCode.ENTER && Handsontable.helper.keyCode.SHIFT):
			...
			cells: 
				function (row, col, prop) {						
					...
					if (row !== hot.countRows() && col > (lastVisCol - 6)) {
						...
						cellProperties.editor = CustomEditor;
			...
			for (var j = 0; j < parseInt(hot.countRows()); j++) {
					for (var k = 0; k < parseInt(hot.countCols()); k++) {
						...
						if((parseInt(headerLength - k) < 3))  {  //&& (parseInt(j) !== lastVisRow)) {
							...
							// hot.setDataAtCell(parseInt(j), k, 123);
							hot.setCellMeta (parseInt(j), k, 'readOnly', true);
							// console.log(j + " " + lastVisRow);
						} 						
						//LAST COLUMN, LAST THREE ROWS
						// if((parseInt(headerLength - k) < 3) && (parseInt(j) == lastVisRow)) {
